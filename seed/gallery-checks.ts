/**
 * Validates a set of /create addresses (normally seed/gallery.ts, or,
 * for testing only, a file given as the first CLI argument) strictly
 * enough that a clean result guarantees stage 7.3 can insert the
 * resolved specs unmodified. Pure - no filesystem writes, no database
 * access, nothing time-dependent. seed/check.ts and seed/gallery-sync.ts
 * both call checkGalleryUrls so the validation logic lives in exactly
 * one place.
 *
